    ...
    with torch.no_grad():
        ...
        for act in dataset.act_name:
            ...
            generator = dataset.sampling_generator(num_samples=args.num_samp, batch_size=args.bs,t_pre_extra=args.t_pre_extra,
                                                   act=act)
            for traj_np, label, fn_gt, fn_mask_gt, act_pair in generator:
                ...
                act_pair_tmp = tensor(act_pair, device=device)
                act_pair = act_pair_tmp[:, None, None, :].repeat([1, cfg.vae_specs['n_action'], args.nk, 1]).reshape([-1, 2])

                ##
                fn_mask_gt_tmp = tensor(fn_mask_gt[:, t_his:], device=device)
                dim_fnmask = fn_mask_gt_tmp.shape
                fn_mask_gt = fn_mask_gt_tmp[:, None, None, :].repeat([1, cfg.vae_specs['n_action'], args.nk, 1]).reshape(-1, args.nk, dim_fnmask[-1])#B k T
                ...
                X = traj[:t_his]
                Y = traj[t_his:]#T Bk D
                Y_r, ce_loss_rec = model.sample_prior(X, label)
