...
class DatasetNTU(Dataset):
    ...
    def __init__(self, mode, t_his=25, t_pred=100, actions='all', use_vel=False,
        ...
        if 'acts' in kwargs.keys() and kwargs['acts'] is not None:
            ...
            self.act_name = np.array([paramUtil.ntu_action_enumerator[i] for i in self.act_label])
        ...
        # if use_vel:
        #     self.traj_dim += 3
        self.data_file = f'../data/ntu_{self.min_len}_{self.max_len}_wact_candi_{self.mode}.npz'
    ...
    def process_data(self):
        ...
        # plt.hist(seq_n, density=True, bins=100)
        # plt.savefig(f'./ntu_seq_n_hist_{self.mode}.jpg')
        ...
        if self.is_6d:
            ...
            for key in data_f.keys():
                ...
                for i, seq in enumerate(data_tmp):
                    ...
                    # rot1 = utils.compute_rotation_matrix_from_ortho6d(rot6d)
