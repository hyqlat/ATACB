...
def loss_function(X, Y_r, Y, mu, logvar, pmu, plogvar, fn_mask, dct_m, idct_m, act2_label, gene_mclass):
    #act2_label: B, gene_mclass: T B classnum
    ...
    MSE = (Y_r - Y).pow(2).sum(dim=-1).transpose(0, 1)[fn_mask == 1].mean()#只取有用部分mask为1
    ...
    # crossentro
    celossfunction = nn.CrossEntropyLoss(reduction="mean")
    act2_label = act2_label.unsqueeze(0).repeat(gene_mclass.shape[0], 1).reshape((-1))
    classnum = gene_mclass.shape[-1]
    gene_mclass = gene_mclass.reshape((-1, classnum))
    celoss = celossfunction(gene_mclass, act2_label)


    # regularization
    # div =
    if len(lambdas) == 4:
        loss_r = lambdas[0] * MSE + lambdas[1] * MSE_v1 + lambdas[1] * MSE_v2 + lambdas[2] * KLD + lambdas[3] * celoss
    else:
        loss_r = lambdas[0] * MSE + lambdas[1] * MSE_v1 + lambdas[2] * MSE_v2 + lambdas[3] * KLD + lambdas[4] * celoss
    # loss_r = MSE
    return loss_r, np.array([loss_r.item(), MSE.item(), MSE_v1.item(), MSE_v2.item() , KLD.item(), celoss.item()])
...
def train(epoch):
    ...
    total_num_sample = 0
    train_grad = 0
    loss_names = ['TOTAL', 'MSE', 'DCT_smooth', 'Lastframe_smooth', 'KLD', 'classCE']
    ...
    for traj_np, label, fn, fn_mask, act_pair in generator:
        ...
        X = traj[:t_his]#T B C
        Y = traj[t_his:]
        Y_r, mu, logvar, pmu, plogvar, gene_mclass, ce_loss_rec = model(X, Y, label, fn, True)

        act2_label = torch.argmax(label, dim=1)#B
        loss, losses = loss_function(X, Y_r, Y, mu, logvar, pmu, plogvar, fn_mask, dct_m, idct_m, act2_label, gene_mclass)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def test(epoch):
    ...
    with torch.no_grad():
        ...
        for traj_np, label, fn, fn_mask, act_pair in generator:
            ...
            X = traj[:t_his]
            Y = traj[t_his:]
            Y_r, mu, logvar, pmu, plogvar, gene_mclass, ce_loss_rec = model(X, Y, label, fn)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg_cc', default='grab_act_cc')
    ...
    #classconti的cfg
    cfg_cc = Config(args.cfg_cc, test=args.test)
    #
    ...
    #读入classi参数并冻结
    cc_path = cfg_cc.cc_model_path % 500
    print('loading model from checkpoint: %s' % cc_path)
    model_cc = pickle.load(open(cc_path, "rb"))
    
    model.conticl.load_state_dict(model_cc['model_dict'])
    ...
    optimizer = optim.Adam(filter(lambda x: x.requires_grad, model.parameters()), lr=cfg.vae_lr)
